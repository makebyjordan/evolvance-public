...
import type { InvoiceIn } from "@/app/actions/invoices-in-actions";
import { deleteInvoiceIn } from "@/app/actions/invoices-in-actions";
import { db } from "@/lib/firebase";
import { collection, onSnapshot, query, orderBy, Timestamp } from "firebase/firestore";
import { Skeleton } from "@/components/ui/skeleton";
import { Badge } from "@/components/ui/badge";

export function InvoicesInClientPage() {
  const [invoices, setInvoices] = useState<InvoiceIn[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [isFormOpen, setIsFormOpen] = useState(false);
  const [isAlertOpen, setIsAlertOpen] = useState(false);
  const [selectedInvoice, setSelectedInvoice] = useState<InvoiceIn | null>(null);
  const { toast } = useToast();

  useEffect(() => {
    const q = query(collection(db, "invoicesIn"), orderBy("createdAt", "desc"));
    const unsubscribe = onSnapshot(q, 
      (querySnapshot) => {
        const invoicesData: InvoiceIn[] = [];
  ...
  const handleDeleteClick = (invoice: InvoiceIn) => {
    setSelectedInvoice(invoice);
    setIsAlertOpen(true);
  };

  const handleEditClick = (invoice: InvoiceIn) => {
    setSelectedInvoice(invoice);
    setIsFormOpen(true);
  };
  ...
  const confirmDelete = async () => {
    ...
    const result = await deleteInvoiceIn(selectedInvoice.id);
