...
import { savePresupuesto, type Presupuesto } from "@/app/actions/presupuestos-actions";
...
const formSchema = z.object({
    presupuestoNumber: z.string().min(1, "El número de presupuesto es requerido."),
    clientName: z.string().min(1, "El nombre del cliente es requerido."),
    clientInfo: z.string().min(1, "La información del cliente es requerida."),
    date: z.date({ required_error: "La fecha es requerida." }),
    status: z.enum(['Borrador', 'Enviado', 'Aceptado', 'Rechazado']),
    items: z.array(presupuestoItemSchema).min(1, "Debe haber al menos un item."),
    notes: z.string().optional(),
});

type PresupuestoFormValues = z.infer<typeof formSchema>;
...
export function PresupuestoForm({ isOpen, setIsOpen, onFormSubmit, presupuesto }: PresupuestoFormProps) {
  ...
  useEffect(() => {
    if (isEditing && presupuesto) {
      form.reset({
        presupuestoNumber: presupuesto.presupuestoNumber,
        clientName: presupuesto.clientName,
        clientInfo: presupuesto.clientInfo,
        date: new Date(presupuesto.date),
      ...
      form.reset({
        presupuestoNumber: `PRE-${Date.now()}`,
        clientName: "",
        clientInfo: "",
        date: new Date(),
        status: "Borrador",
  ...
  }, [isEditing, presupuesto, form]);


  const onSubmit = async (values: PresupuestoFormValues) => {
    const presupuestoData = {
      ...values,
      id: isEditing ? presupuesto!.id : undefined,
    };
    ...
    if (result.success) {
      toast({
        title: `Presupuesto ${isEditing ? 'Actualizado' : 'Creado'}`,
        description: `El presupuesto "${values.presupuestoNumber}" ha sido guardado.`,
      });
      onFormSubmit();
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-4xl bg-card max-h-screen overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="font-headline text-primary">{isEditing ? 'Editar Presupuesto' : 'Nuevo Presupuesto'}</DialogTitle>
          <DialogDescription>Rellena los campos para {isEditing ? 'actualizar' : 'crear'} un presupuesto.</DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            <FormField control={form.control} name="presupuestoNumber" render={({ field }) => ( <FormItem><FormLabel>Nº Presupuesto</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>)} />
            ...
            <FormField control={form.control} name="notes" render={({ field }) => ( <FormItem><FormLabel>Notas</FormLabel><FormControl><Textarea rows={3} placeholder="Validez de la oferta, etc." {...field} /></FormControl><FormMessage /></FormItem>)} />

            <DialogFooter className="sticky bottom-0 bg-card py-4 -mx-6 px-6 border-t"><Button type="button" variant="outline" onClick={() => handleOpenChange(false)}>Cancelar</Button><Button type="submit" disabled={form.formState.isSubmitting}>{form.formState.isSubmitting ? 'Guardando...' : (isEditing ? 'Guardar Cambios' : 'Crear Presupuesto')}</Button></DialogFooter>
          </form>
        </Form>
      </DialogContent>
    </Dialog>
